...
const SS_SUB_ISSUE_BUNDLES: SsSubIssueBundle[] = [
  ...
  },
  {
    ...
    contextWriteUp: "About 1.49 million foreign workers live in Singapore, and the question is whether they belong to the diverse society they have physically built or remain functionally excluded from it. The tension is direct: the same workers who put up the housing, MRT lines and Marina Bay skyline are typically tied by visa to a single employer and housed in dormitories deliberately set apart from citizen residential life. The official position is that migrant workers are essential partners in development, with employment, housing and welfare arrangements designed to balance economic needs against the realities of large temporary populations. Advocacy voices argue the opposite — that visa terms tying workers to one employer make formal complaints economically punishing, so what looks like inclusion in practice operates as managed exclusion. Many citizens hold a sympathetic-but-distanced view, recognising unfair treatment during the COVID-19 dormitory outbreaks while still hesitating at proposals to integrate worker housing into HDB estates; international comparative evidence warns that economic inclusion without legal and social inclusion tends to be unsustainable over time. The issue matters because Singapore's diverse-society narrative is tested most sharply at exactly the point where economic interdependence meets residential and social distance, and the answer given here defines who actually counts as part of the society being built.",
