 ...
 *  must equal `targetMarks`. The model is told this constraint in the prompt,
 *  but we never trust the output — this normaliser is the last line of defence
 *  so a paper's printed mark allocation is never exceeded.
 *
 *  - Rebalances proportionally when sums differ.
 *  - Honours `lockedIndices` (e.g. SBQ "assertion" parts locked at 8 marks).
 *  - Floors all questions at 1 mark.
 *  - Logs and clamps when the blueprint is malformed (locked marks already
 *    exceed budget, or budget < num_questions). */
function normalizeSectionMarks(
  ...
  // Lock contributions (and clamp them to >= 1).
  let lockedSum = 0;
  ...
  if (flexBudget < flexCount) {
    // Cannot give every flexible question >= 1 mark. Clamp to 1 each, log, and
    // accept the overshoot — caller's blueprint is malformed.
  ...
  // Take the model's relative weighting on the flexible slots and rescale.
  ...
  // Distribute the leftover (or trim the overshoot) one mark at a time.
  if (scaledSum < flexBudget) {
    // Add to the lowest-mark questions first, preserving relative weighting.
    ...
    // Trim from the highest-mark questions first; never drop below 1.
    ...
    while (scaledSum > flexBudget) {
      ...
      k++;
      if (k > flexCount * (flexBudget + 5)) break; // safety
    }
  }
