...
const SS_SUB_ISSUE_BUNDLES: SsSubIssueBundle[] = [
  ...
  },
  {
    ...
    contextWriteUp: "HDB public housing is one of the most ambitious nation-building projects any state has undertaken, and the question is how far it has actually produced a shared Singaporean identity rather than simply provided shelter. Since 1960, HDB has built more than a million flats, housing over 80% of residents and giving roughly 90% home ownership, and the Government frames the EIP, void decks and shared estate amenities as the everyday infrastructure that makes ethnic mixing routine. Sociologists like Chua Beng Huat support this view, arguing that the 'heartland' figure in political speech and popular culture is unimaginable without growing up in an HDB estate. Younger Singaporeans tell a more strained story, pointing to record million-dollar resale flats, long BTO waits and tighter PR/citizen ballots that suggest housing now sorts people by timing and family wealth. IPS researchers find class divisions in social networks are now more rigid than racial ones, while the OECD notes that high-coverage public-housing systems can drift from citizenship asset to wealth marker as prices outpace incomes. The issue matters because if public housing stops binding citizens to a common experience, the policy with the widest daily reach into Singaporean lives loses its identity-shaping work.",
