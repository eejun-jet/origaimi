...
const SS_SUB_ISSUE_BUNDLES: SsSubIssueBundle[] = [
  ...
  },
  {
    ...
    contextWriteUp: "Singapore's open economy — with external trade running at roughly 3.5 times GDP and dozens of free-trade agreements in force — has lifted national prosperity, but the question is whether ordinary workers share in those gains or absorb most of the costs. The tension is between aggregate openness as a national strategy and the lived experience of competing in a labour market continually exposed to global shifts. The official position is that staying deeply plugged into the global economy raises consumer choice and creates jobs, and that re-skilling and wage-support schemes are designed as cushions that make exposure to global competition manageable. Labour economists qualify the claim, noting that uptake of training schemes skews towards already-better-educated workers and that the bottom income quintile has seen the slowest real-wage growth despite headline gains. Individual experience captures the gap directly, with mid-career workers in offshorable industries describing every new trade agreement as another forced restart of their working lives. Comparative cases — including votes in advanced economies driven partly by the argument that integration's gains had not reached ordinary voters — show what can happen politically when distribution lags too far behind growth, even where overall inequality after transfers is falling. The issue matters because the open-economy bet shapes every Singaporean's working life, and whether the model holds depends on whether workers continue to believe it works for them.",
  ...
  },
  {
    ...
    contextWriteUp: "Globalisation and Singaporean cultural identity raises the question of whether a small, deeply connected society can keep producing a distinctive culture of its own while consuming media made elsewhere. The tension is that the same openness that powers Singapore's economy also brings overwhelming flows of foreign content — K-drama, Hollywood, English-language streaming — into the same homes, schools and screens where local identity is supposed to form. The official position treats local cultural production as something that must be actively defended, channelling resources into heritage programming, local content quotas and language campaigns rather than trusting market forces to do that work. Many cultural commentators agree but go further, arguing that Singaporean identity is best understood as deliberately hybrid — built out of East Asian, South Asian, Malay-Indonesian and Anglo-American influences reworked into distinctive forms — so globalisation is the soil in which local identity grows rather than a force eroding it. Younger audiences increasingly follow non-English regional content ahead of locally produced television, prompting fears that habit will outpace identification with home-grown culture. Comparative international evidence concludes that cultural identity in the streaming era is a deliberate political and budgetary choice, not a natural inheritance, with smaller markets risking becoming consumers rather than producers of culture. The issue matters because the institutions that do the hybridising — neighbourhood schools, kopitiams, void decks, local broadcasters — are themselves under pressure, and the answer will shape what Singaporean culture means to the next generation.",
