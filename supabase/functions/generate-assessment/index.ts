...
const SS_SUB_ISSUE_BUNDLES: SsSubIssueBundle[] = [
  ...
  },
  {
    ...
    contextWriteUp: "Managing race and religion in Singapore is the question of whether harmony between communities is produced by deliberate state policy or by everyday social mixing — and what happens when the two are pulled apart. The Government's position is that policy is doing the heavy lifting: the Ethnic Integration Policy sets racial quotas in every HDB block, and the Maintenance of Religious Harmony Act lets the Minister for Home Affairs pre-empt religious leaders who mix faith with politics or stoke inter-group hostility. Individual experience suggests harmony is also negotiated case by case, as with the 2021 lifting of the tudung restriction for public-sector nurses after extensive community consultation, welcomed by some as overdue and questioned by others as overly mediated. Comparative cases show how contested the same ground is elsewhere: Quebec's Bill 21 banning religious symbols for teachers, police and judges sparked challenges from Muslim, Sikh and Jewish groups arguing the law discriminates in effect even if not by name. IPS survey data lands in between, with 78% of residents agreeing policy underwrites harmony and 62% reporting close cross-racial friendships, suggesting policy and contact reinforce rather than substitute for each other. The issue matters because Singapore's small, religiously plural society has no margin for the kind of inter-group breakdown seen elsewhere, and the model of harmony chosen now shapes what is normal for the next generation.",
