...
const SS_SUB_ISSUE_BUNDLES: SsSubIssueBundle[] = [
  ...
  },
  {
    ...
    contextWriteUp: "National Service is the two-year, compulsory military, police or civil-defence stint that every male Singapore citizen and second-generation male PR has owed the state since the National Service (Amendment) Act of 1967, followed by up to a decade of reservist cycles. The Government, from Goh Keng Swee in 1967 through PM Lee at the NS50 commemoration, has consistently framed NS as both a defence necessity for a small state and a deliberate citizenship-forging rite that mixes young men across race, language and class. Many NSmen agree that the shared hardship gives them a 'thick, lived' sense of being Singaporean before anything else. Others — including IPS researchers — point out that women, new citizens who arrive after the eligible age, and first-generation PRs are entirely outside this experience, so NS may deepen citizenship for some while widening the gap with everyone else. Comparative reporting from the BBC on conscription in South Korea, Taiwan and Singapore notes that younger citizens across all three societies, increasingly mobile and transnational, are starting to contest whether the model can hold. The issue matters because NS still consumes years of every male citizen's life and remains the single clearest policy answer to what the obligations of Singaporean citizenship actually are.",
