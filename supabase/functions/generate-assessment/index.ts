...
const SS_SUB_ISSUE_BUNDLES: SsSubIssueBundle[] = [
  ...
  },
  {
    ...
    contextWriteUp: "Foreign immigrants — new citizens and permanent residents drawn from across Asia and beyond — sit at the centre of an unresolved debate about whether newcomers can genuinely integrate into Singaporean life. The tension is that the country needs continued immigration to stop the citizen population from shrinking, but cohesion depends on whether newcomers become Singaporean rather than parallel residents. The official position treats integration as a 'two-way process': structured programmes introduce newcomers to local history and norms, while existing citizens are urged to be open in return. Many locals judge integration narrowly by visible markers — language, accent, food habits, willingness to bear NS — and a majority feel newcomers are not making enough effort. New citizens and PRs measure it differently, by workplace acceptance and freedom to keep home-country ties, with only a minority reporting close Singaporean friends outside their own group. Sociologists argue this 'everyday friction' is normal and that genuine belonging is a generational outcome settled by the children of newcomers, while comparative international evidence finds integration is strongest where formal programmes are matched by routine personal contact. The issue matters because Singapore's demographic and economic future depends on continued immigration, but its social cohesion depends on whether newcomers actually become Singaporean.",
